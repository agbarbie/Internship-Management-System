...
exports.getInternById = exports.getInterns = void 0;
const db_1 = __importDefault(require("../db"));
// ── GET ALL INTERNS ───────────────────────────────────────────
const getInterns = async (req, res) => {
    try {
        const result = await db_1.default.query('SELECT * FROM v_intern_overview ORDER BY overall_score DESC');
...
const getInternById = async (req, res) => {
    try {
        const intern = await db_1.default.query(`SELECT u.id, u.first_name, u.last_name, u.email, u.phone,
       ...
       WHERE u.id = $1 AND u.role = 'intern'`, [req.params.id]);
        if (intern.rows.length === 0) {
            res.status(404).json({ error: 'Intern not found.' });
            return;
        }
        const tasks = await db_1.default.query(`SELECT * FROM v_task_summary
       WHERE intern_name = (SELECT first_name || ' ' || last_name FROM users WHERE id = $1)
       ORDER BY due_date DESC`, [req.params.id]);
        const feedback = await db_1.default.query(`SELECT f.rating, f.comment, f.created_at,
