...
// GET /api/tasks/:id  — get single task with submissions and feedback
...
// POST /api/tasks  — create a new task (supervisor only)
...
// POST /api/tasks/:id/submit  — intern submits a task
router.post('/:id/submit', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (task.rows.length > 0) {
      await pool.query(
        `INSERT INTO notifications (user_id, type, title, message, link)
         VALUES ($1, 'task_assigned', 'Task Submitted for Review', $2, $3)`,
...
// PATCH /api/tasks/:id/review  — supervisor approves or requests revision
router.patch('/:id/review', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
...
router.get('/:id/files', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
